...
export interface UseCheckoutResult extends UseCheckoutState {
  /** Begin a checkout: initiate, then poll to a terminal/retryable state. */
  start: (input: InitiateCheckoutInput) => Promise<void>
  /** Cancel any in-flight initiation/polling. */
  cancel: () => void
  reset: () => void
}

/**
 * Headless checkout controller. This is the shared behavior surface both the web
 * widget and (conceptually) native views render. It owns no markup — it drives
 * the @mitumba/pay state machine and exposes phase/status/error.
 */
export function useCheckout(
  config: MitumbaPayConfig | MitumbaPay,
): UseCheckoutResult {
